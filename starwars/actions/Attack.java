...
public class Attack extends SWAffordance implements SWActionInterface {
	 ...
	 *  <li>The <code>SWActor a</code> is holding a <code>Lightsaber</code> with insufficient force level</li>
	 * </ul>
	 * <p>
	 ...
	 * 
	 * @author  tdngu38 - added description for if SWActor is holding a Lightsaber
	...
	public void act(SWActor a) {
		...
		if (targetIsActor && (a.getTeam() == targetActor.getTeam())) { //don't attack SWActors in the same team
			...
			if (itemCarried != null) {//if the actor is carrying an item 
					...
					if (itemCarried instanceof LightSaber){
						if (a instanceof ForceActor){
							int aForceLevel = ((ForceActor) a).getForceLevel();
								if (aForceLevel < 70){
								attackWithNonWea(a, targetActor, itemCarried);
							}
								else {
								attackWithWeapon(a, target, itemCarried, energyForAttackWithWeapon);
							}
						}
						else {
							attackWithNonWea(a, targetActor, itemCarried);
				...
				attackWithHands(a, target, energyForAttackWithWeapon);
			}
	...
	/**
	 * Perform attack with bare hands
	 * @param a the actor performs this action
	 * @param target the target receives this action
	 * @param energy energy used by a
	 */
	private void attackWithHands(SWActor a, SWEntityInterface target, int energy){
		target.takeDamage((a.getHitpoints()/20) + 1); // a bare-handed attack doesn't do much damage.
		a.takeDamage(2*energy); // actor uses energy. It's twice as tiring as using a weapon
	}
	
	/**
	 * Perform attack when the actor is holding an item but cannot use it as a weapon
	 * @param a the actor performs this action
	 * @param targetActor the target receives this action
	 * @param itemCarried the item carried by a
	 */
	private void attackWithNonWea(SWActor a, SWActor targetActor, SWEntityInterface itemCarried){
	...
	/**
	 * perform attack with a weapon
	 * @param a the actor performs this action
	 * @param target the target receives this action
	 * @param itemCarried item carried by the actor
	 * @param energy energy used by a
	 */
	private void attackWithWeapon(SWActor a, SWEntityInterface target, SWEntityInterface itemCarried, int energy){
			...
			a.takeDamage(energy); // actor uses energy to attack
	}
}
